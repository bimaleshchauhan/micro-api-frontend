...
import { onRemoveFromWishlist, onViewProfile, onAddToCart, onRemoveFromCart, onCreateAddress, onPlaceOrder, getOrders, deleteAddress  } from '../store/actions'
...
const Profile = () => {
    ...
    const { user, wishlist, cart, address } = useSelector(state => state.userReducer);
    ...
    const editAddress = (id) => {
        if (address && address.length > 0 && id) {
            const findAddress = address.find(add => add._id===id)
            if (findAddress) {
                setEditAddress(findAddress)
                setCountry(findAddress.country)
                setPostalCode(findAddress.postalCode)
                setCity(findAddress.city)
                setStreet(findAddress.street)
            }
        }
    ...
    const handleAddress = () => {
        ...
        if(Array.isArray(address) && address.length && !_editAddress){
                return  <div className="row">
                <div className="col-12">
                <h1 className="text-4xl text-indigo-900 py-2"> Shopping Profile </h1>
                </div>
                <div className="col" style={{ height: '15rem' }}>
                    <label>Your Delivery Address </label>
                    <AddressComponent address={address} deleteAddress= {(id)=>dispatch(deleteAddress(id))} editAddress = {(id) => editAddress(id)} />
                </div>  
                ...
                <div className="col-12">
                    <nav>
                        <div className="nav nav-tabs" id="nav-tab" role="tablist">
                            ...
                            <a className="nav-link" onClick={() =>  dispatch(getOrders())} style={{ color: '#3680B4'}} id="nav-orders-tab" data-toggle="tab" href="#nav-orders" role="tab" aria-controls="nav-orders" aria-selected="false"><i className="fas fa-list-alt mr-3"></i>Orders</a>
                        </div>
                    </nav>
