...
export const onGetProducts = (payload) => async(dispatch) => {
  ...
  export const deleteAddress = (AddressId) => async(dispatch) => {
    try {
        const response = await DeleteData('/customer/address/'+AddressId);
        console.log("response", response)
        dispatch({ type:  Action.REMOVE_ADDRESS, payload: response.data });
  ...
  export const editAddress = ({AddressId, street, postalCode,city,country }) => async(dispatch) => {
    try {
      const response = await PutData('/customer/address/'+AddressId, {
        street, postalCode,city,country 
      });
      
      dispatch({ type:  Action.EDIT_ADDRESS, payload: response.data });
  ...
  export const getOrders = () => async(dispatch) => {
    ...
    try {
        ...
        dispatch({ type:  Action.GET_ORDERS, payload: response.data });
